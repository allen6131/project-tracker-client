...
const DownloadDesktopLink: React.FC<DownloadDesktopLinkProps> = ({ variant = 'button', className = '' }) => {
  ...
  const releasesHref = `https://github.com/${process.env.REACT_APP_GITHUB_REPO || 'allen6131/project-tracker'}/releases/latest`;
  ...
  return (
    <a
      ...
      onClick={(e) => {
        // If we can't detect platform-specific artifact, fallback to releases page
        if (!href || href.endsWith('/releases/latest')) {
          e.preventDefault();
          window.open(releasesHref, '_blank', 'noopener');
        }
      }}
    >
